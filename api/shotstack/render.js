export default async function handler(req, res) {
  ...
  const body = await req.body; // Vercel streams for Node, but we can just read text/json:
  const json = typeof body === "string" ? JSON.parse(body || "{}") : req.body || {};
  const edit = buildEditJson(json); // see helper below
  ...
  return res.status(200).json({ renderId: data.response?.id || data.id || data?.data?.id || null });
